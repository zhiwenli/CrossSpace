...
measureBox.setAttribute('style', 'left:220px;top:60px;');
...
function stopDrawing() {
    //return; //dont remove cesium handler
    ...
    // console.log("----->", leftClick, dbClick); //??存储的事件并非原自定义的事件层级
}
...
function lineCut_Mercator(curPositions){
    ...
    for(var j=0;j<curPositions.length-1;j++){
        ...
        var cartoLeft = Cesium.Cartographic.fromCartesian(curPositions[j]);
...
//lzW add. 2018-1-22 10:44:23
function measureBoxShow(){
    var measureBox = document.getElementById('measureBox');
    var profile = document.getElementById('profileBox');
    var cb_profile = document.getElementById('cb_profile');
    var cb_mouseControl = document.getElementById('cb_mouseControl');
